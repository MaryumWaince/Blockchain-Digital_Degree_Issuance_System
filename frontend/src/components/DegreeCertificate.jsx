...
const DegreeCertificate = ({ studentDID }) => {
  ...
  <p><strong>Vice Chancellor Signature:</strong></p>
  <VcSignatureDisplay />
</div>

        <p>📜 <strong>University Logo </strong></p>
      </section>
    </div>
  );
};
...
const DegreeCertificate = ({ studentDID }) => {
  ...
  return (
    <div style={{
      ...
      <section>
        <h3>General Information</h3>
        <p><strong>Student DID:</strong> {did || 'N/A'}</p>
        <p><strong>Name:</strong> {studentName || 'N/A'}</p>
        <p><strong>Email:</strong> {studentEmail || 'N/A'}</p>
        <p><strong>Degree:</strong> {degreeName || 'N/A'}</p>
        <p><strong>CGPA:</strong> {cgpa ?? 'Not Available'}</p>
        <p><strong>Result Declaration Date:</strong> {resultDeclarationDate ? new Date(resultDeclarationDate).toLocaleDateString() : 'Not Available'}</p>
        <p><strong>Degree Issue Date:</strong> {issuedOn ? new Date(issuedOn).toLocaleDateString() : 'Not Available'}</p>
      </section>
